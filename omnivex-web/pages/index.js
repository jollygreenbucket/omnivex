  ...
  return (
    <div className={`card card-sm ${className}`} style={{
      ...
      <div style={{
        ...
        fontWeight: 400, color: accent || 'var(--text)', lineHeight: 1.1,
      }}>{value ?? '—'}</div>
...
export default function Omnivex() {
  ...
  // perfChart defined here using portData directly — no hoisting issue
  ...
  allocation.forEach(a => {
    actualAlloc[a.tier] = totalValue > 0 ? +((a.total_value / totalValue) * 100).toFixed(1) : 0
  })
  ...
  return (
    <>
      ...
      <header style={{
        ...
        background: 'rgba(8,9,13,.92)', backdropFilter: 'blur(16px)',
        borderBottom: '1px solid var(--ink-4)', padding: '0 36px',
        display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 58,
      }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 18 }}>
          <span style={{ fontFamily: 'var(--font-serif)', fontSize: 21, color: 'var(--gold)', letterSpacing: '.04em', fontStyle: 'italic' }}>Omnivex</span>
          <span className="label" style={{ letterSpacing: '.2em', opacity: .5 }}>Paquette Capital</span>
        </div>
          ...
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(8, 1fr)', gap: 10, marginBottom: 28 }}>
              {[
                ...
                ['SPY', `${(run?.spy_daily_pct||0)>=0?'+':''}${fmt(run?.spy_daily_pct)}%`, null, (run?.spy_daily_pct||0)>=0?'#2dd4a0':'#e05252', 'anim-2'],
                ...
                <StatCard key={label} label={label} value={value} sub={sub} accent={ac} className={cls} />
              ))}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 28 }}>
              ...
              <div className="card anim-2" style={{ borderTop: '2px solid var(--alpha)' }}>
                <div className="label" style={{ color: 'var(--alpha)', marginBottom: 16 }}>Top Buy Candidates</div>
                {scores.filter(s => ['BUY','ADD'].includes(s.action)).slice(0,6).map(s => (
                  <div key={s.ticker} onClick={() => setFocusTicker(s.ticker)}
                    style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: '1px solid var(--ink-2)', cursor: 'pointer' }}>
                    <div>
                      <span style={{ fontWeight: 400, fontSize: 13 }}>{s.ticker}</span>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: tierCol(s.tier), marginLeft: 8 }}>{tierLabel(s.tier)}</span>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div className={`${scoreCls(s.omnivex_score)} mono`} style={{ fontSize: 13 }}>{fmt(s.omnivex_score)}</div>
                      <ScoreBar value={s.omnivex_score} />
                    </div>
                  </div>
                ))}
                {scores.filter(s=>['BUY','ADD'].includes(s.action)).length===0 && (
                  <div style={{ color: 'var(--silver-2)', fontSize: 12 }}>None at current thresholds</div>
                )}
              </div>

              {/* Reduces */}
              <div className="card anim-2" style={{ borderTop: '2px solid var(--hedge)' }}>
                <div className="label" style={{ color: 'var(--hedge)', marginBottom: 16 }}>Reduce / Rotate</div>
                {scores.filter(s=>['REDUCE','REMOVE','ROTATE'].includes(s.action)).slice(0,6).map(s => (
                  <div key={s.ticker} onClick={() => setFocusTicker(s.ticker)}
                    style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: '1px solid var(--ink-2)', cursor: 'pointer' }}>
                    <span style={{ fontSize: 13 }}>{s.ticker}</span>
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                      <span className={`c-${(s.action||'').toLowerCase()} mono`} style={{ fontSize: 10 }}>{s.action}</span>
                      <span className={`${scoreCls(s.omnivex_score)} mono`} style={{ fontSize: 13 }}>{fmt(s.omnivex_score)}</span>
                    </div>
                  </div>
                ))}
              ...
              <div className="card anim-2">
                <div className="label" style={{ color: 'var(--gold)', marginBottom: 16 }}>System Alerts</div>
                <div style={{ background: 'var(--ink-2)', borderRadius: 6, padding: '10px 14px', marginBottom: 12 }}>
                  <div className="label" style={{ marginBottom: 6 }}>Mode Proximity</div>
                  {[['Alpha', run?.alpha_trigger_count||0, 6, 'var(--alpha)'],['Hedge', run?.hedge_trigger_count||0, 5, 'var(--hedge)']].map(([name, count, total, color]) => (
                    <div key={name} style={{ marginBottom: 8 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                        <span style={{ fontSize: 11, color }}>{name}</span>
                        <span className="mono" style={{ fontSize: 10, color: 'var(--silver-2)' }}>{count}/{total}</span>
                      </div>
                      <div style={{ height: 2, background: 'var(--ink-4)', borderRadius: 1 }}>
                        <div style={{ height: '100%', borderRadius: 1, background: color, width: `${(count/total)*100}%` }} />
                      </div>
                    </div>
                  ))}
                </div>
                {scores.filter(s=>s.forensic_flag).length > 0 ? (
                  <>
                    <div className="label" style={{ color: 'var(--hedge)', marginBottom: 8 }}>⚠ Forensic Flags</div>
                    {scores.filter(s=>s.forensic_flag).slice(0,5).map(s => (
                      <div key={s.ticker} style={{ padding: '7px 0', borderBottom: '1px solid var(--ink-2)' }}>
                        <div style={{ fontWeight: 400, marginBottom: 2 }}>{s.ticker}</div>
                        <div style={{ fontSize: 10, color: 'var(--silver-2)' }}>{(s.forensic_detail||'').replace(/\|/g,' · ')}</div>
                      </div>
                    ))}
                  </>
                ) : (
              ...
              <div className="card anim-3" style={{ marginBottom: 28 }}>
                ...
                <ResponsiveContainer width="100%" height={72}>
                  <BarChart data={distribution} barSize={40} barGap={4}>
                    <XAxis dataKey="band" tick={{ fill: '#5a6278', fontSize: 10, fontFamily: 'var(--font-mono)' }} axisLine={false} tickLine={false} />
                    <YAxis hide />
                    <Tooltip contentStyle={{ background: '#0e1018', border: '1px solid #1c2130', borderRadius: 6, fontSize: 11, fontFamily: 'var(--font-mono)' }} />
                    <Bar dataKey="count" radius={[3,3,0,0]}>
                      {distribution.map((d,i) => (
                        <Cell key={i} fill={scoreCol(d.band==='Breakout'?85:d.band==='Overweight'?75:d.band==='Maintain'?65:d.band==='Underweight'?55:40)} fillOpacity={.8} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
            ...
            <div className="card anim-4" style={{ padding: 0, overflow: 'hidden' }}>
              <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--ink-3)', display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                <input className="pq-input" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search ticker / sector..." style={{ width: 200 }} />
                <div style={{ display: 'flex', gap: 6 }}>
                  {['ALL','SMART_CORE','TACTICAL','SPECULATIVE'].map(t => (
                    <button key={t} className={`filter-pill ${tierF===t?'active':''}`} onClick={() => setTierF(t)}>{t.replace('_',' ')}</button>
                  ))}
                </div>
                <select className="pq-input" value={actionF} onChange={e => setActionF(e.target.value)} style={{ width: 110 }}>
                  {['ALL','BUY','ADD','HOLD','REDUCE','REMOVE','ROTATE','MONITOR'].map(a => <option key={a} value={a}>{a}</option>)}
                </select>
                <span className="label" style={{ marginLeft: 'auto' }}>{scores.length} results</span>
              </div>
              <div style={{ overflowX: 'auto' }}>
                <table className="pq-table">
                  <thead>
                    <tr>
                      {[['ticker','Ticker'],['sector','Sector'],['tier','Tier'],['omnivex_score','Score'],['qtech','QTech'],['psos','PSOS'],['signal_conf','Sig. Conf'],['action','Action'],['suggested_weight_pct','Weight']].map(([key,lbl]) => (
                  ...
                  <tbody>
                    {scores.map(s => (
                      <tr key={s.ticker} onClick={() => setFocusTicker(s.ticker)}>
                        ...
                        <td><span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: tierCol(s.tier) }}>{tierLabel(s.tier)}</span></td>
                        <td>
                          <span className={`${scoreCls(s.omnivex_score)} mono`} style={{ fontSize: 13 }}>{fmt(s.omnivex_score)}</span>
                          <ScoreBar value={s.omnivex_score} />
                        </td>
                        ...
                        <td className={`c-${(s.action||'monitor').toLowerCase()} mono`} style={{ fontSize: 11 }}>{s.action||'—'}</td>
                        <td className="mono" style={{ color: 'var(--silver-2)' }}>{s.suggested_weight_pct ? `${fmt(s.suggested_weight_pct)}%` : '—'}</td>
                        <td>
                          {s.forensic_flag && <span className="badge badge-hedge" style={{ marginRight: 4 }}>Forensic</span>}
                          {(s.flags||'').includes('EARNINGS_IMMINENT') && <span className="badge badge-gold">Earnings</span>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          <div>
            {holdings.length === 0 ? (
              <div className="card" style={{ textAlign: 'center', padding: 60 }}>
                <div style={{ fontFamily: 'var(--font-serif)', fontSize: 22, color: 'var(--gold)', marginBottom: 12 }}>No Holdings</div>
                <div style={{ color: 'var(--silver-2)', fontSize: 13 }}>Add holdings via Schwab sync or directly in Neon.</div>
              </div>
            ) : (
              <>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 10, marginBottom: 28 }}>
                  <StatCard label="Total Value" value={fmtM(totalValue)} accent="var(--gold)" />
                  <StatCard label="Unrealized P&L" value={fmtM(totalPnl)} sub={`${totalPnlPct>=0?'+':''}${fmt(totalPnlPct)}%`} accent={totalPnl>=0?'var(--alpha)':'var(--hedge)'} />
                ...
                {mounted && perfChart.length > 0 && (
                  <div className="card" style={{ marginBottom: 20 }}>
                    <div className="label" style={{ marginBottom: 16 }}>Performance vs SPY — Cumulative %</div>
                    <ResponsiveContainer width="100%" height={180}>
                      <LineChart data={perfChart}>
                        <XAxis dataKey="date" tickFormatter={fmtDate} tick={{ fill: '#5a6278', fontSize: 10, fontFamily: 'var(--font-mono)' }} axisLine={false} tickLine={false} interval="preserveStartEnd" />
                        <YAxis tick={{ fill: '#5a6278', fontSize: 10, fontFamily: 'var(--font-mono)' }} axisLine={false} tickLine={false} width={36} tickFormatter={v => `${v>0?'+':''}${v.toFixed(1)}%`} />
                        <Tooltip content={<ChartTip />} />
                        <ReferenceLine y={0} stroke="#1c2130" strokeDasharray="4 4" />
                        <Line type="monotone" dataKey="portfolio" name="Omnivex" stroke={accent} strokeWidth={2} dot={false} />
                        <Line type="monotone" dataKey="spy" name="SPY" stroke="#363d52" strokeWidth={1.5} strokeDasharray="4 4" dot={false} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                )}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 20 }}>
                  <div className="card">
                    <div className="label" style={{ marginBottom: 16 }}>Allocation — Target vs Actual</div>
                      ...
                      return (
                        <div key={tier} style={{ marginBottom: 16 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: col }}>{tierLabel(tier)}</span>
                            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--silver-2)' }}><span style={{ color: 'var(--text)' }}>{fmt(actual)}%</span> / {fmt(target)}% target</span>
                          </div>
                          ...
                          <div style={{ height: 3, background: 'var(--ink-3)', borderRadius: 2 }}>
                            <div style={{ height: '100%', width: `${Math.min(100,actual)}%`, background: actual>target+5?'var(--hedge)':actual<target-5?'var(--core)':col, borderRadius: 2 }} />
                          </div>
                        </div>
                      )
                    })}
                  ...
                  <div className="card">
                    <div className="label" style={{ marginBottom: 16 }}>Tier Performance</div>
                    <table className="pq-table">
                      <thead><tr><th>Tier</th><th>Positions</th><th>Value</th><th>P&L</th><th>Avg Score</th></tr></thead>
                      <tbody>
                        {allocation.map(a => (
                          <tr key={a.tier}>
                            <td style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: tierCol(a.tier) }}>{tierLabel(a.tier)}</td>
                            <td className="mono">{a.positions}</td>
                            <td className="mono">{fmtM(a.total_value)}</td>
                            <td className={`mono ${a.total_pnl>=0?'c-pos':'c-neg'}`}>{fmtM(a.total_pnl)}</td>
                            <td className={`mono ${scoreCls(a.avg_score)}`}>{fmt(a.avg_score)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                      ...
                      <tbody>
                        {holdings.map(h => (
                          <tr key={h.ticker} onClick={() => setFocusTicker(h.ticker)}>
                            ...
                            <td style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: tierCol(h.tier) }}>{tierLabel(h.tier)}</td>
                            ...
                            <td className={`mono ${(h.unrealized_pnl||0)>=0?'c-pos':'c-neg'}`}>{fmtM(h.unrealized_pnl)}</td>
                            <td className={`mono ${(h.unrealized_pnl_pct||0)>=0?'c-pos':'c-neg'}`}>{fmtPct(h.unrealized_pnl_pct)}</td>
                            <td className={`mono ${scoreCls(h.omnivex_score)}`}>{fmt(h.omnivex_score)}</td>
                            <td className={`c-${(h.action||'monitor').toLowerCase()} mono`} style={{ fontSize: 10 }}>{h.action||'—'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ...
                  <div style={{ overflowX: 'auto' }}>
                    <table className="pq-table">
                      ...
                      <tbody>
                        {trades.map(t => (
                          <tr key={t.id}>
                            <td className="mono" style={{ color: 'var(--silver-2)' }}>{fmtDate(t.trade_date)}</td>
                            <td style={{ fontWeight: 400 }}>{t.ticker}</td>
                            <td className={`c-${(t.action||'').toLowerCase()} mono`} style={{ fontSize: 10 }}>{t.action}</td>
                            ...
                            <td style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: tierCol(t.tier) }}>{tierLabel(t.tier)}</td>
                            <td><span className={`badge ${modeCls(t.mode)}`}>{modeLabel(t.mode)}</span></td>
                            <td style={{ color: 'var(--silver-2)' }}>{t.notes||'—'}</td>
                          </tr>
                        ))}
                        {trades.length===0 && (
                          <tr><td colSpan={10} style={{ color: 'var(--silver-2)', textAlign: 'center', padding: 32 }}>No trades logged yet</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
        ...
        {tab === 'history' && (
          <div>
            <div className="card anim-1" style={{ marginBottom: 20 }}>
              <div className="label" style={{ marginBottom: 16 }}>Market Context — 90 Days</div>
              {mounted && (
                <ResponsiveContainer width="100%" height={180}>
                  <AreaChart data={modeHistory || []}>
                    ...
                    <XAxis dataKey="run_date" tickFormatter={fmtDate} tick={{ fill: '#5a6278', fontSize: 10, fontFamily: 'var(--font-mono)' }} axisLine={false} tickLine={false} interval="preserveStartEnd" />
                    <YAxis tick={{ fill: '#5a6278', fontSize: 10, fontFamily: 'var(--font-mono)' }} axisLine={false} tickLine={false} width={30} />
                    <Tooltip content={<ChartTip />} />
                    <ReferenceLine y={18} stroke="#2dd4a040" strokeDasharray="3 3" />
                    <ReferenceLine y={22} stroke="#e0525240" strokeDasharray="3 3" />
                    <Area type="monotone" dataKey="vix" name="VIX" stroke={accent} strokeWidth={1.5} fill="url(#vixG)" dot={false} />
                    <Line type="monotone" dataKey="ad_ratio" name="A/D" stroke="#6eb3f7" strokeWidth={1} dot={false} strokeDasharray="3 3" />
                  </AreaChart>
                </ResponsiveContainer>
              )}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div className="card anim-2">
                <div className="label" style={{ marginBottom: 14 }}>Top Score Movers</div>
                {(movers||[]).map(m => (
                  <div key={m.ticker} onClick={() => { setFocusTicker(m.ticker); setTab('signals') }}
                    style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--ink-2)', cursor: 'pointer' }}>
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                      <span style={{ fontSize: 13 }}>{m.ticker}</span>
                      <span className={`${scoreCls(m.omnivex_score)} mono`} style={{ fontSize: 11 }}>{fmt(m.omnivex_score)}</span>
                    </div>
                    <span className="mono" style={{ fontSize: 12, color: (m.score_delta||0)>=0?'var(--alpha)':'var(--hedge)' }}>
                      {(m.score_delta||0)>=0?'+':''}{fmt(m.score_delta)}
                    </span>
                  </div>
                ))}
                {!movers?.length && <div style={{ color: 'var(--silver-2)', fontSize: 12 }}>Requires 2+ daily runs</div>}
              </div>
              ...
              <div className="card anim-2" style={{ padding: 0, overflow: 'hidden' }}>
                ...
                <div style={{ overflowY: 'auto', maxHeight: 360 }}>
                  <table className="pq-table">
                    <thead><tr><th>Date</th><th>Mode</th><th>VIX</th><th>SPY</th><th>Scored</th></tr></thead>
                    <tbody>
                      {(runHistory||[]).map(r => (
                        <tr key={r.run_date}>
                          <td className="mono" style={{ color: 'var(--silver-2)' }}>{fmtDate(r.run_date)}</td>
                          <td><span className={`badge ${modeCls(r.mode)}`} style={{ fontSize: 8 }}>{modeLabel(r.mode)}</span></td>
                          <td className="mono">{fmt(r.vix)}</td>
                          <td className={`mono ${(r.spy_daily_pct||0)>=0?'c-pos':'c-neg'}`}>{(r.spy_daily_pct||0)>=0?'+':''}{fmt(r.spy_daily_pct)}%</td>
                          <td className="mono" style={{ color: 'var(--silver-2)' }}>{r.tickers_scored}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      {focusTicker && (
        <div style={{
          position: 'fixed', right: 0, top: 58, bottom: 0, width: 320,
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            <span style={{ fontFamily: 'var(--font-serif)', fontSize: 24, color: 'var(--gold)', fontStyle: 'italic' }}>{focusTicker}</span>
            <button onClick={() => { setFocusTicker(null); setTickerHist(null) }}
              style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--silver-2)', fontSize: 20 }}>×</button>
          </div>
          {!tickerHist ? (
            ...
            <>
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <ResponsiveContainer width="100%" height={110}>
                    <LineChart data={tickerHist.history}>
                      <XAxis dataKey="run_date" tickFormatter={fmtDate} tick={{ fill: '#5a6278', fontSize: 9 }} axisLine={false} tickLine={false} />
                      <YAxis domain={[0,100]} tick={{ fill: '#5a6278', fontSize: 9 }} axisLine={false} tickLine={false} width={24} />
                      <Tooltip content={<ChartTip />} />
                      <Line type="monotone" dataKey="omnivex_score" name="Score" stroke={accent} strokeWidth={2} dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              )}
              {tickerHist.current && (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  {[['Score',fmt(tickerHist.current.omnivex_score)],['Action',tickerHist.current.action||'—'],['QTech',fmt(tickerHist.current.qtech)],['PSOS',fmt(tickerHist.current.psos)],['Signal',fmt(tickerHist.current.signal_conf)],['Tier',tierLabel(tickerHist.current.tier)]].map(([k,v]) => (
                    <div key={k} style={{ background: 'var(--ink-2)', borderRadius: 6, padding: '10px 12px' }}>
              ...
              {tickerHist.history?.length === 0 && (
                <div style={{ color: 'var(--silver-2)', fontSize: 12 }}>No history — first appearance today.</div>
              )}
            </>
          )}
        </div>
